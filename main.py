...
gps.send_command(gps.SET_POS_FIX_4S)
...
def button0_pressed_handler(pin):
...
def button1_pressed_handler(pin):
    global screen, last_time
    new_time = utime.ticks_ms()
    # if it has been more that 1/5 of a second since the last event, we have a new event
    if (new_time - last_time) > 200:
...
keyA.irq(trigger=machine.Pin.IRQ_FALLING, handler = button0_pressed_handler)
keyB.irq(trigger=machine.Pin.IRQ_FALLING, handler = button1_pressed_handler)
